...
class VideoDataset_images_with_motion_features(data.Dataset):
    ...
    def __init__(self, data_dir, data_dir_3D ,filename_path, transform, database_name, crop_size, feature_type):
        ...
        if database_name == 'KoNViD-1k' or database_name == 'LIVE_VQC' or database_name == 'YouTubeUGC':
            dataInfo = open(filename_path, 'r').readlines()
            n = len(dataInfo)
            video_names = [line.split(',')[0] for line in dataInfo]
            score = [float(line.split(',')[-1]) for line in dataInfo]
    ...
    def __getitem__(self, idx):
        if self.database_name == 'KoNViD-1k' or self.database_name == 'LIVE_VQC' \
            or self.database_name == 'YouTubeUGC' :
        ...
        if self.database_name == 'KoNViD-1k' or self.database_name == 'LSVQ_train' or self.database_name == 'LSVQ_test' or self.database_name == 'LSVQ_test_1080p' or self.database_name == 'LIVE_VQC':
            video_length_read = 8
        elif self.database_name == 'YouTubeUGC':
...
class VideoDataset_images_VQA_dataset_with_motion_features(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(video_length_read):
            if self.database_name == 'YouTubeUGC':
