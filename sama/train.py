...
def finetune_epoch(ft_loader, model, model_ema, optimizer, scheduler, device, epoch=-1, awp=None):
    model.train()
    ...
    for i, data in enumerate(ft_loader):
        ...
        loss.backward()
        if awp is not None:
            loss = awp.attack_backward(video, y)
            logger.info(f"Epoch {epoch}, Iteration {i + 1}/{len(ft_loader)}: AWP Loss: {loss.item():.4f}")
            loss.backward()
            awp._restore()
        optimizer.step()
        scheduler.step()
...
def main():
    ...
    parser.add_argument("-o", "--opt", type=str, 
                        default="./options/fast-SAMA-train.yml", help="the option file")
    ...
    for key in opt["data"]:
        if key.startswith("train"):
            train_dataset = getattr(datasets, opt["data"][key]["type"])(opt["data"][key]["args"], stype=stype)
    ...
    for key, train_dataset in train_datasets.items():
        train_loaders[key] = torch.utils.data.DataLoader(train_dataset, 
    ...
    awp = AWP(model, loss_fn, optimizer, gamma=args.gamma) if args.awp else None
    for epoch in range(opt["num_epochs"]):
        logger.info(f"Finetune Epoch {epoch}:")

        for key, train_loader in train_loaders.items():
            finetune_epoch(
                train_loader, model, model_ema, optimizer, scheduler, device, epoch, awp=awp)
